...
//		Alternativ: Menupunkte gleichen Dateinamens einander erg�nzen.
//		
//		Positions �berpr�fen und menupunkt hinten anh�ngen
//		in menu und template_id �berpr�fen
//			-> Ansatz: beim exportieren nicht die template_id exportieren sondern den Namen/Titel und dannach abgleichen, wenn nicht mit 1 bzw ersten template importieren
		...
		if ($count > 0):
			for($i = 0; $i < $count; $i++):
				foreach($xml->menu->item[$i] as $itemkey => $item): //XML-Object einlesen
					if (is_array($item)): //Pr�fen ob XML Atribut ein Array ist
		...
		foreach($import as $im):
			echo "<pre>$im</pre><hr />\n";
//			$err = mysql_query($im); //SQL-Statements ausf�hren
