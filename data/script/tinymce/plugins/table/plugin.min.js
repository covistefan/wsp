...
define("tinymce/tableplugin/Plugin", [
	...
	function Plugin(editor) {
		...
		function removeTableClass(cla) {
			return cla ? cla.replace(/mce-item-table$/, '') : "";
		}
		...
		function buildTableClassList(tableclassList) {
			var tableclassListItems = [{text: 'None', value: ''}];
			var classListText;
			if (typeof(tableclassList) == "string") {
				tinymce.util.XHR.send({
					url: tableclassList,
					success: function(text3) {
						classListText = tinymce.util.JSON.parse(text3);
						tinymce.each(classListText, function(classes) {
							tableclassListItems.push({
								text: classes.text,
								value: classes.value
			...
			return tableclassListItems;
		}

		function classListChangeHandler(e) {
			var textCtrl = win.find('#classes');
			
			win.find('#classes').value(removeTableClass(textCtrl.value()) + ' ' + e.control.value());
					
		}

		function tableDialog() {
			var dom = editor.dom, tableElm, data, createNewTable;
			var tableclassListItems = editor.settings.table_class_list;
			var tableclassListText;
			var classListCtrl;	
	...
	//				values: buildClassList(data.classes),
					values: buildTableClassList(tableclassListItems),
...
//			var tableclassListItems = [{text: 'None', value: ''}];
//			tinymce.each(tableclassList, function(classes) {
//				tableclassListItems.push({
//					text: classes.text,
//					value: classes.value
////					selected: classesValue === classes.value
...
//
			...
			data = {
				...
				classes: tableElm ? removeTableClass(dom.getAttrib(tableElm, 'class')) : ''
			};
			...
			win = editor.windowManager.open({
				...
				items: {
					...
					items: [
						...
						classListCtrl,
						{
							name: 'classes', type: 'textbox', columns: 1, maxWidth: 600, label: 'Klassen', value: data.classes
						}	
					],
		...
		function cellDialog() {
			...
			var cellclassListItems = editor.settings.table_cell_class_list;
			var cellclassListText;
			var classListCtrl;	
			...
			if (editor.settings.class_list !== false) {
				classListCtrl = {
					...
					label: 'verfügbare Klasse ',
	//				values: buildClassList(data.classes),
					values: buildTableClassList(cellclassListItems),
					onselect:  classListChangeHandler
				};
			}
			...
			data = {
				...
				classes: cellElm ? removeTableClass(dom.getAttrib(cellElm, 'class')) : ''
			};
				...
				onsubmit: function() {
					...
					editor.undoManager.transact(function() {
						each(cells, function(cellElm) {
							...
							editor.dom.setAttrib(cellElm, 'class', data.classes);
		...
		function rowDialog() {
			...
			var rowclassListItems = editor.settings.table_row_class_list;
			var rowclassListText;
			var classListCtrl;	
			...
			if (editor.settings.class_list !== false) {
				classListCtrl = {
					...
					label: 'verfügbare Klasse ',
	//				values: buildClassList(data.classes),
					values: buildTableClassList(rowclassListItems),
					onselect:  classListChangeHandler
				};
			}
			...
			data = {
				...
				classes: rowElm ? removeTableClass(dom.getAttrib(rowElm, 'class')) : ''
			};
				...
				onsubmit: function() {
					...
					editor.undoManager.transact(function() {
						...
						each(rows, function(rowElm) {
							...
							editor.dom.setAttrib(rowElm, 'class', data.classes);
